/*
 * File: Humvee 05 Camera
 * Name: Humvee05Camera.cs
...
namespace VehiclesControl
{
	// public class Humvee05Camera
	public class Humvee05Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The humvee 05 transform")]
			// Transform _humvee05
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _humvee05.position.y plus _height
			float _desiredHeight = _humvee05.position.y + _height;
			...
			// transform position is _humvee05.position
			transform.position = _humvee05.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _humvee05.InverseTransformDirection(_humvee05.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _humvee05.eulerAngles.y plus 180
				_tempVector3.y = _humvee05.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _humvee05.eulerAngles.y
				_tempVector3.y = _humvee05.eulerAngles.y;
			...
			float _autoCamControl = _humvee05.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Humvee05Camera

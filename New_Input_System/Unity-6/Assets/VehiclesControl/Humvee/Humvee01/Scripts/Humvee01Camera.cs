/*
 * File: Humvee 01 Camera
 * Name: Humvee01Camera.cs
...
namespace VehiclesControl
{
	// public class Humvee01Camera
	public class Humvee01Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The humvee 01 transform")]
			// Transform _humvee01
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _humvee01.position.y plus _height
			float _desiredHeight = _humvee01.position.y + _height;
			...
			// transform position is _humvee01.position
			transform.position = _humvee01.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _humvee01.InverseTransformDirection(_humvee01.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _humvee01.eulerAngles.y plus 180
				_tempVector3.y = _humvee01.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _humvee01.eulerAngles.y
				_tempVector3.y = _humvee01.eulerAngles.y;
			...
			float _autoCamControl = _humvee01.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Humvee01Camera

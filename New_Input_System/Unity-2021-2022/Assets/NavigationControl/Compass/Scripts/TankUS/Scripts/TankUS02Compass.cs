/*
*  Name: Tank US 02 Compass
*  File: TankUS02Compass.cs
...
namespace NavigationControl
{
	// public class TankUS02Compass
	public class TankUS02Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		public static TankUS02Compass _tankUS02Compass;

		// private void Start
		private void Start()
		{
			// _tankUS02Compass
			_tankUS02Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component TankUS02Compass is enabled
				GetComponent<TankUS02Compass>().enabled = true;

				// Debug Log TankUS02 compass is enabled
			...
			{
				// Debug Log TankUS02 compass is disabled
				...
				// Get Component TankUS02Compass is not enabled
				GetComponent<TankUS02Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _tankUS02 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_tankUS02.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _tankUS02 forward vector

			// Vector3 forward is _tankUS02 transform forward
			Vector3 forward = _tankUS02.transform.forward;

			// Zero out the Y of _tankUS02 forward vector to only get the direction in the X,Z 
	...
	} // close public class TankUS02Compass

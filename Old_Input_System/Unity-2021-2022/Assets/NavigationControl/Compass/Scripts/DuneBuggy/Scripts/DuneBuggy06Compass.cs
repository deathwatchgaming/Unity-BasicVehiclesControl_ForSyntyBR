/*
*  Name: Dune Buggy 06 Compass
*  File: DuneBuggy06Compass.cs
...
namespace NavigationControl
{
	// public class DuneBuggy06Compass
	public class DuneBuggy06Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The DuneBuggy06 transform")]
		...
		public static DuneBuggy06Compass _duneBuggy06Compass;

		// private void Start
		private void Start()
		{
			// _duneBuggy06Compass
			_duneBuggy06Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component DuneBuggy06Compass is enabled
				GetComponent<DuneBuggy06Compass>().enabled = true;

				// Debug Log DuneBuggy06 compass is enabled
			...
			{
				// Debug Log DuneBuggy06 compass is disabled
				...
				// Get Component DuneBuggy06Compass is not enabled
				GetComponent<DuneBuggy06Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _duneBuggy06 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_duneBuggy06.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _duneBuggy06 forward vector

			// Vector3 forward is _duneBuggy06 transform forward
			Vector3 forward = _duneBuggy06.transform.forward;

			// Zero out the Y of _duneBuggy06 forward vector to only get the direction in the X,Z 
	...
	} // close public class DuneBuggy06Compass

/*
*  Name: Tank US 06 Compass
*  File: TankUS06Compass.cs
...
namespace NavigationControl
{
	// public class TankUS06Compass
	public class TankUS06Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The TankUS06 transform")]
			// Transform _tankUS06
		...
		public static TankUS06Compass _tankUS06Compass;

		// private void Start
		private void Start()
		{
			// _tankUS06Compass
			_tankUS06Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component TankUS06Compass is enabled
				GetComponent<TankUS06Compass>().enabled = true;

				// Debug Log TankUS06 compass is enabled
			...
			{
				// Debug Log TankUS06 compass is disabled
				...
				// Get Component TankUS06Compass is not enabled
				GetComponent<TankUS06Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _tankUS06 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_tankUS06.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _tankUS06 forward vector

			// Vector3 forward is _tankUS06 transform forward
			Vector3 forward = _tankUS06.transform.forward;

			// Zero out the Y of _tankUS06 forward vector to only get the direction in the X,Z 
	...
	} // close public class TankUS06Compass

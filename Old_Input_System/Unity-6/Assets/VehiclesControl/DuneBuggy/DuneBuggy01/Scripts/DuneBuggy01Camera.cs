/*
 * File: Dune Buggy 01 Camera
 * Name: DuneBuggy01Camera.cs
...
namespace VehiclesControl
{
	// public class DuneBuggy01Camera
	public class DuneBuggy01Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The dune buggy 01 transform")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _duneBuggy01.position.y plus _height
			float _desiredHeight = _duneBuggy01.position.y + _height;
			...
			// transform position is _duneBuggy01.position
			transform.position = _duneBuggy01.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _duneBuggy01.InverseTransformDirection(_duneBuggy01.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _duneBuggy01.eulerAngles.y plus 180
				_tempVector3.y = _duneBuggy01.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _duneBuggy01.eulerAngles.y
				_tempVector3.y = _duneBuggy01.eulerAngles.y;
			...
			float _autoCamControl = _duneBuggy01.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class DuneBuggy01Camera

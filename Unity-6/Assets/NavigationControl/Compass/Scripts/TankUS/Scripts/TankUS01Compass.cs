/*
*  Name: Tank US 01 Compass
*  File: TankUS01Compass.cs
...
namespace NavigationControl
{
	// public class TankUS01Compass
	public class TankUS01Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		public static TankUS01Compass _tankUS01Compass;

		// private void Start
		private void Start()
		{
			// _tankUS01Compass
			_tankUS01Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component TankUS01Compass is enabled
				GetComponent<TankUS01Compass>().enabled = true;

				// Debug Log TankUS01 compass is enabled
			...
			{
				// Debug Log TankUS01 compass is disabled
				...
				// Get Component TankUS01Compass is not enabled
				GetComponent<TankUS01Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _tankUS01 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_tankUS01.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _tankUS01 forward vector

			// Vector3 forward is _tankUS01 transform forward
			Vector3 forward = _tankUS01.transform.forward;

			// Zero out the Y of _tankUS01 forward vector to only get the direction in the X,Z 
	...
	} // close public class TankUS01Compass

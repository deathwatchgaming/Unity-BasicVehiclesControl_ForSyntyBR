/*
 * File: Dune Buggy 02 Camera
 * Name: DuneBuggy02Camera.cs
...
namespace VehiclesControl
{
	// public class DuneBuggy02Camera
	public class DuneBuggy02Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The dune buggy 02 transform")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _duneBuggy02.position.y plus _height
			float _desiredHeight = _duneBuggy02.position.y + _height;
			...
			// transform position is _duneBuggy02.position
			transform.position = _duneBuggy02.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _duneBuggy02.InverseTransformDirection(_duneBuggy02.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _duneBuggy02.eulerAngles.y plus 180
				_tempVector3.y = _duneBuggy02.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _duneBuggy02.eulerAngles.y
				_tempVector3.y = _duneBuggy02.eulerAngles.y;
			...
			float _autoCamControl = _duneBuggy02.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class DuneBuggy02Camera

/*
 * File: Sedan 05 Camera
 * Name: Sedan05Camera.cs
...
namespace VehiclesControl
{
	// public class Sedan05Camera
	public class Sedan05Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The sedan 05 transform")]
			// Transform _sedan05
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _sedan05.position.y plus _height
			float _desiredHeight = _sedan05.position.y + _height;
			...
			// transform position is _sedan05.position
			transform.position = _sedan05.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _sedan05.InverseTransformDirection(_sedan05.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _sedan05.eulerAngles.y plus 180
				_tempVector3.y = _sedan05.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _sedan05.eulerAngles.y
				_tempVector3.y = _sedan05.eulerAngles.y;
			...
			float _autoCamControl = _sedan05.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Sedan05Camera

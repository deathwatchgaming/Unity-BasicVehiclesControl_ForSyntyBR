...
namespace VehiclesControl
{
	// public class Sedan02Camera
	public class Sedan02Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The sedan 02 transform")]
			// Transform _sedan02
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _sedan02.position.y plus _height
			float _desiredHeight = _sedan02.position.y + _height;
			...
			// transform position is _sedan02.position
			transform.position = _sedan02.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _sedan02.InverseTransformDirection(_sedan02.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _sedan02.eulerAngles.y plus 180
				_tempVector3.y = _sedan02.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _sedan02.eulerAngles.y
				_tempVector3.y = _sedan02.eulerAngles.y;
			...
			float _autoCamControl = _sedan02.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Sedan02Camera

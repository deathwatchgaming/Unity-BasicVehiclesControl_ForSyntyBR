/*
 * File: Tank RU 01 Camera
 * Name: TankRU01Camera.cs
...
namespace VehiclesControl
{
	// public class TankRU01Camera
	public class TankRU01Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The tank ru 01 transform")]
			// Transform _tankRU01
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _tankRU01.position.y plus _height
			float _desiredHeight = _tankRU01.position.y + _height;
			...
			// transform position is _tankRU01.position
			transform.position = _tankRU01.position;
		...
		private void FixedUpdate()
		{
			// Vector3 _localVelocity is _tankRU01 InverseTransformDirection _tankRU01 GetComponent Rigidbody velocity
			Vector3 _localVelocity = _tankRU01.InverseTransformDirection(_tankRU01.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _tankRU01.eulerAngles.y plus 180
				_tempVector3.y = _tankRU01.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is_tankRU01.eulerAngles.y
				_tempVector3.y = _tankRU01.eulerAngles.y;
			...
			// float autoCamControl is_tankRU01 GetComponent Rigidbody velocity magnitude
			float _autoCamControl = _tankRU01.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class TankRU01Camera

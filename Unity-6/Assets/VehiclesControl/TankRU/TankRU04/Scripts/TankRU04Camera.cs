/*
 * File: Tank RU 04 Camera
 * Name: TankRU04Camera.cs
...
namespace VehiclesControl
{
	// public class TankRU04Camera
	public class TankRU04Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The tank ru 04 transform")]
			// Transform _tankRU04
			[SerializeField] private Transform _tankRU04;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _tankRU04.position.y plus _height
			float _desiredHeight = _tankRU04.position.y + _height;
			...
			// transform position is _tankRU04.position
			transform.position = _tankRU04.position;
		...
		private void FixedUpdate()
		{
			// Vector3 _localVelocity is _tankRU04 InverseTransformDirection _tankRU04 GetComponent Rigidbody velocity
			Vector3 _localVelocity = _tankRU04.InverseTransformDirection(_tankRU04.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _tankRU04.eulerAngles.y plus 180
				_tempVector3.y = _tankRU04.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is_tankRU04.eulerAngles.y
				_tempVector3.y = _tankRU04.eulerAngles.y;
			...
			// float autoCamControl is_tankRU04 GetComponent Rigidbody velocity magnitude
			float _autoCamControl = _tankRU04.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class TankRU04Camera

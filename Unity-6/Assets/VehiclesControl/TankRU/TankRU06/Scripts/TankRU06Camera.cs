...
namespace VehiclesControl
{
	// public class TankRU06Camera
	public class TankRU06Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _tankRU06.position.y plus _height
			float _desiredHeight = _tankRU06.position.y + _height;
			...
			// transform position is _tankRU06.position
			transform.position = _tankRU06.position;
		...
		private void FixedUpdate()
		{
			// Vector3 _localVelocity is _tankRU06 InverseTransformDirection _tankRU06 GetComponent Rigidbody velocity
			Vector3 _localVelocity = _tankRU06.InverseTransformDirection(_tankRU06.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _tankRU06.eulerAngles.y plus 180
				_tempVector3.y = _tankRU06.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is_tankRU06.eulerAngles.y
				_tempVector3.y = _tankRU06.eulerAngles.y;
			...
			// float autoCamControl is_tankRU06 GetComponent Rigidbody velocity magnitude
			float _autoCamControl = _tankRU06.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class TankRU06Camera

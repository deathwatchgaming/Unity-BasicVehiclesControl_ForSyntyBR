/*
 * File: Humvee 02 Camera
 * Name: Humvee02Camera.cs
...
namespace VehiclesControl
{
	// public class Humvee02Camera
	public class Humvee02Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The humvee 02 transform")]
			// Transform _humvee02
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _humvee02.position.y plus _height
			float _desiredHeight = _humvee02.position.y + _height;
			...
			// transform position is _humvee02.position
			transform.position = _humvee02.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _humvee02.InverseTransformDirection(_humvee02.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _humvee02.eulerAngles.y plus 180
				_tempVector3.y = _humvee02.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _humvee02.eulerAngles.y
				_tempVector3.y = _humvee02.eulerAngles.y;
			...
			float _autoCamControl = _humvee02.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Humvee02Camera

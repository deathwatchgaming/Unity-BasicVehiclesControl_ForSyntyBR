/*
 * File: Humvee 04 Camera
 * Name: Humvee04Camera.cs
...
namespace VehiclesControl
{
	// public class Humvee04Camera
	public class Humvee04Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The humvee 04 transform")]
			// Transform _humvee04
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _humvee04.position.y plus _height
			float _desiredHeight = _humvee04.position.y + _height;
			...
			// transform position is _humvee04.position
			transform.position = _humvee04.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _humvee04.InverseTransformDirection(_humvee04.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _humvee04.eulerAngles.y plus 180
				_tempVector3.y = _humvee04.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _humvee04.eulerAngles.y
				_tempVector3.y = _humvee04.eulerAngles.y;
			...
			float _autoCamControl = _humvee04.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Humvee04Camera

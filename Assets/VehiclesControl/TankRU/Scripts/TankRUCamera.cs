...
namespace VehiclesControl
{
	// public class TankRUCamera
	public class TankRUCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _tankRU.position.y plus _height
			float _desiredHeight = _tankRU.position.y + _height;
			...
			// transform position is _tankRU.position
			transform.position = _tankRU.position;
		...
		private void FixedUpdate()
		{
			// Vector3 _localVelocity is _tankRU InverseTransformDirection _tankRU GetComponent Rigidbody velocity
			Vector3 _localVelocity = _tankRU.InverseTransformDirection(_tankRU.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _tankRU.eulerAngles.y plus 180
				_tempVector3.y = _tankRU.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is_tankRU.eulerAngles.y
				_tempVector3.y = _tankRU.eulerAngles.y;
			...
			// float acc is_tankRU GetComponent Rigidbody velocity magnitude
			float _acc = _tankRU.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class TankRUCamera

/*
 * File: Dune Buggy Camera
 * Name: DuneBuggyCamera.cs
...
namespace VehiclesControl
{
	// public class DuneBuggyCamera
	public class DuneBuggyCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The dune buggy transform")]
			// Transform _duneBuggy
			[SerializeField] private Transform _duneBuggy;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _duneBuggy.position.y plus _height
			float _desiredHeight = _duneBuggy.position.y + _height;
			...
			// transform position is _duneBuggy.position
			transform.position = _duneBuggy.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _duneBuggy.InverseTransformDirection(_duneBuggy.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _duneBuggy.eulerAngles.y plus 180
				_tempVector3.y = _duneBuggy.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _duneBuggy.eulerAngles.y
				_tempVector3.y = _duneBuggy.eulerAngles.y;
			...
			// float acc
			float _acc = _duneBuggy.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class DuneBuggyCamera

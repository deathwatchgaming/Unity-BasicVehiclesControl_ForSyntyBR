/*
 * File: DuneBuggy Speedometer
 * Name: DuneBuggySpeedometer.cs
...
namespace VehiclesControl
{
    // public enum DuneBuggySpeedUnit
	public enum DuneBuggySpeedUnit
	{
		mph,
		kmh	

	} // close public enum DuneBuggySpeedUnit

	// public class DuneBuggySpeedometer 
	public class DuneBuggySpeedometer : MonoBehaviour
	{
		// Speedometer
		[Header("Speedometer")]
		...
		[Header("Speed Text")]
			...
			// DuneBuggySpeedUnit _speedUnit	
			[SerializeField] private DuneBuggySpeedUnit _speedUnit;
		...
		private void Start()
		{
			...
			// GameObject _interfaceIMG01Object is FindInActiveObjectByName DuneBuggy_SpeedoGuage
			GameObject _interfaceIMG01Object = FindInActiveObjectByName("DuneBuggy_SpeedoGuage");

			// _interfaceIMG01Object SetActive is false
			_interfaceIMG01Object.SetActive(false);

			// GameObject _interfaceIMG02Object is FindInActiveObjectByName DuneBuggy_SpeedoNeedle
			GameObject _interfaceIMG02Object = FindInActiveObjectByName("DuneBuggy_SpeedoNeedle");

			// _interfaceIMG02Object SetActive is false
			_interfaceIMG02Object.SetActive(false);	

			// GameObject _interfaceTextObject is FindInActiveObjectByName DuneBuggy_SpeedText
			GameObject _interfaceTextObject = FindInActiveObjectByName("DuneBuggy_SpeedText");

			// _interfaceTextObject SetActive is false
			_interfaceTextObject.SetActive(false);			

			// GameObject _interfaceParentObject is FindInActiveObjectByName DuneBuggySpeedometer
			GameObject _interfaceParentObject = FindInActiveObjectByName("DuneBuggySpeedometer");
		...
		private void Update()
		{
			// if _speedUnit equals DuneBuggySpeedUnit.mph
			if (_speedUnit == DuneBuggySpeedUnit.mph)
			{
			...
			} // close if _speedUnit equals DuneBuggySpeedUnit.mph
			...
			// if _inDuneBuggy and Input GetKey KeyCode _exitKey
			if (_inDuneBuggy && Input.GetKey(_exitKey))
			{
				// _inDuneBuggy is false
				_inDuneBuggy = false;

			} // close if _inDuneBuggy and Input GetKey KeyCode _exitKey
		...
		private void OnTriggerStay(Collider other)
		{
			...
			// if not _inDuneBuggy and gameObject tag is Player and Input GetKey KeyCode _enterKey
			...
			} // close if not _inDuneBuggy and gameObject tag is Player and Input GetKey KeyCode _enterKey
	...
	} // close public class DuneBuggySpeedometer

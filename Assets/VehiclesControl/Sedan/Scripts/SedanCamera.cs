...
namespace VehiclesControl
{
	// public class SedanCamera
	public class SedanCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

            [Tooltip("The sedan transform")]
		    // Transform _sedan
			[SerializeField] private Transform _sedan;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _sedan.position.y plus _height
			float _desiredHeight = _sedan.position.y + _height;
			...
			// transform position is _sedan.position
			transform.position = _sedan.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _sedan.InverseTransformDirection(_sedan.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
	            ...
	            // _tempVector3.y is _sedan.eulerAngles.y plus 180
				_tempVector3.y = _sedan.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _sedan.eulerAngles.y
				_tempVector3.y = _sedan.eulerAngles.y;
	        ...
	        // float acc
			float _acc = _sedan.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class SedanCamera

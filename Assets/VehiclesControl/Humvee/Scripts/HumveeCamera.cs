...
namespace VehiclesControl
{
	// public class HumveeCamera
	public class HumveeCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
			...
			// Transform _humvee
			[SerializeField] private Transform _humvee;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _humvee.position.y plus _height
			float _desiredHeight = _humvee.position.y + _height;
			...
			// transform position is _humvee.position
			transform.position = _humvee.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _humvee.InverseTransformDirection(_humvee.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _humvee.eulerAngles.y plus 180
				_tempVector3.y = _humvee.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _humvee.eulerAngles.y
				_tempVector3.y = _humvee.eulerAngles.y;
			...
			// float acc
			float _acc = _humvee.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class HumveeCamera

/*
*  Name: Humvee 05 Compass
*  File: Humvee05Compass.cs
...
namespace NavigationControl
{
	// public class Humvee05Compass
	public class Humvee05Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Humvee05 transform")]
			// Transform _humvee05
			[SerializeField] private Transform _humvee05;
		...
		public static Humvee05Compass _humvee05Compass;

		// private void Start
		private void Start()
		{
			// _humvee05Compass
			_humvee05Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Humvee05Compass is enabled
				GetComponent<Humvee05Compass>().enabled = true;

				// Debug Log Humvee05 compass is enabled
			...
			{
				// Debug Log Humvee05 compass is disabled
				...
				// Get Component Humvee05Compass is not enabled
				GetComponent<Humvee05Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _humvee05 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_humvee05.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _humvee05 forward vector

			// Vector3 forward is _humvee05 transform forward
			Vector3 forward = _humvee05.transform.forward;

			// Zero out the Y of _humvee05 forward vector to only get the direction in the X,Z 
	...
	} // close public class Humvee05Compass

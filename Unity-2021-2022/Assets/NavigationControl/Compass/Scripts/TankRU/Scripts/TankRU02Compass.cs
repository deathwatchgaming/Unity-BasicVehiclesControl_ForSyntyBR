/*
*  Name: Tank RU 02 Compass
*  File: TankRU02Compass.cs
...
namespace NavigationControl
{
	// public class TankRU02Compass
	public class TankRU02Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The TankRU02 transform")]
			// Transform _tankRU02
		...
		public static TankRU02Compass _tankRU02Compass;

		// private void Start
		private void Start()
		{
			// _tankRU02Compass
			_tankRU02Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component TankRU02Compass is enabled
				GetComponent<TankRU02Compass>().enabled = true;

				// Debug Log TankRU02 compass is enabled
			...
			{
				// Debug Log TankRU02 compass is disabled
				...
				// Get Component TankRU02Compass is not enabled
				GetComponent<TankRU02Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _tankRU02 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_tankRU02.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _tankRU02 forward vector

			// Vector3 forward is _tankRU02 transform forward
			Vector3 forward = _tankRU02.transform.forward;

			// Zero out the Y of _tankRU02 forward vector to only get the direction in the X,Z 
	...
	} // close public class TankRU02Compass

/*
*  Name: Sedan 05 Compass
*  File: Sedan05Compass.cs
...
namespace NavigationControl
{
	// public class Sedan05Compass
	public class Sedan05Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Sedan05 transform")]
			// Transform _sedan05
			[SerializeField] private Transform _sedan05;
		...
		public static Sedan05Compass _sedan05Compass;

		// private void Start
		private void Start()
		{
			// _sedan05Compass
			_sedan05Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Sedan05Compass is enabled
				GetComponent<Sedan05Compass>().enabled = true;

				// Debug Log Sedan05 compass is enabled
			...
			{
				// Debug Log Sedan05 compass is disabled
				...
				// Get Component Sedan05Compass is not enabled
				GetComponent<Sedan05Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _sedan05 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_sedan05.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _sedan05 forward vector

			// Vector3 forward is _sedan05 transform forward
			Vector3 forward = _sedan05.transform.forward;

			// Zero out the Y of _sedan05 forward vector to only get the direction in the X,Z 
	...
	} // close public class Sedan05Compass
